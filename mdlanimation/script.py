...
def first_pass( commands ):
    ...
    if found_frames:
        if not found_basename:
            print "DEFAULT BASENAME BEING USED. Basename: " + basename
...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
        if command[0] == "vary":
            ...
            start_knob_value = float(knob_scale) / frame_diff

            # setting the values in the knobs value list
            for frame in range(start_frame, end_frame + 1):
                value = start_knob_value * (frame - start_frame)
...
def run(filename):
    ...
    for frame in range(num_frames):
        ...
        if num_frames > 1:
            print "Saved frame in anim/. Filename is " + basename + ("%03d" % frame) + ".png"
