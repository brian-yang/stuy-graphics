...
def notHidden(x0, y0, z0, x1, y1, z1, x2, y2, z2):
    # two vectors that share an endpoint
    A = [x1 - x0, y1 - y0, z1 - z0]
    B = [x2 - x0, y2 - y0, z2 - z0]
    ...
    # find the angle between the normal and the viewing vector
    n_dot_v = N[2] # since V = [0, 0, 1]
    n_magnitude = math.sqrt(pow(N[0], 2) + pow(N[1], 2) + pow(N[2], 2))
    v_magnitude = 1

    if n_magnitude == 0:
        return False
    else:
        theta = math.acos(n_dot_v / (n_magnitude * v_magnitude) )

    return theta > ((3 * math.pi) / 2) or theta < (math.pi/2)
    '''
...
def draw_polygons( points, screen, color ):
    ...
    while point < len(points) - 2:
        ...
        if notHidden(x0, y0, z0, x1, y1, z1, x2, y2, z2):
